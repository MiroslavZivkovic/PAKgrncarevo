...
C
C=======================================================================
      SUBROUTINE D3M30(TAU,DEF,IRAC,LPOCG,LPOC1)
      USE PLAST3D
...
C
      ...
      LCYC1 = LL101 + 10
C
      CALL TAU330(PLAST(LEL),PLAST(LP0),PLAST(LSH),PLAST(LSIG),
     1            PLAST(LEPS),PLAST(LL10),PLAST(LCYC),
     1            PLAS1(LEL1),PLAS1(LP01),PLAS1(LSH1),
     1            PLAS1(LSIG1),PLAS1(LEPS1),PLAS1(LL101),PLAS1(LCYC1),
     1            A(LFUN),TAU,DEF,IRAC)
C
      RETURN
      END
C=======================================================================
      SUBROUTINE TAU330(ELST,P0T,PSH,SIGV,EPSV,DELTAT,CYC,
...
C
  180 K = 1
   ...
   60 CONTINUE 
      ...
      IF  (IT.GT.ITMAX) THEN
          WRITE(3,2000) IT,P0TT,TAU(2),TAU(1),TAU(4)
...
C     RACUNANJE ELST = ELDIM/EL  (EL-STAR)
  190 IF (ISH.EQ.1) GO TO 195
      ...
      ELSE
         IF (GAMAA.GT.TOL) ELSTT = 0.5*DABS(TAU(4))/AMIP0
      ENDIF
...
C
      IF (ISH.EQ.-1) THEN
      WRITE(3,*) '  DOSLO JE DO RAZARANJA MATERIJALA'
      STOP
      ENDIF
