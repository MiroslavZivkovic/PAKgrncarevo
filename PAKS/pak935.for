...
C
C=======================================================================
      SUBROUTINE MASE93(AM,MAXA,ID,EMAS,NEL,NGLOB,NP,NE,NTTOT)
...
C
   ...
   30 CONTINUE
      ...
      END
C=======================================================================
      SUBROUTINE VAUPD3(VA,VA0,ID,RC,EMAS,AA,CC,BB,ISNA,IK,IK1,ALFK,
...
C.. PODACI O PRIPADAJUCIM JEDNACINAMA
      LMILM=0
      DO 135 II=1,3
        KK=II*(MAXCE+2)-MAXCE-1
        LM(KK)=ID(NCK,II)
        ILM(II)=IDC(NLM,II)
        LM(KK+1)=ILM(II)
        IF(LMILM.EQ.0) LMILM=ILM(II)
        K1=KK+1
        IF(NCAA.GT.0)THEN
          DO 132 L=1,MAXCE
          NCV=NELSF(L,NCA)
  132     LM(K1+L)=ID(NCV,II)
        ENDIF
  135 CONTINUE
C
C
...
C
  ...
  700 JD = JR
      ...
      END                 
C=======================================================================
      SUBROUTINE ADDS93(A,C,S,JDIAG,LD,NST,NEL)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
...
C
  ...
  200 CONTINUE
      ...
      END    
C=======================================================================
      FUNCTION NODTRG(NELAB,HE,LD,KLR,NCA,NCVE)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION NELAB(NCVE,*),HE(9,*),LD(*)
      DUM=-1.D20
      DO 15 KK=1,NCVE
      LD(KK)=NELAB(KK,NCA)
        IF(HE(KK,1).GT.DUM)THEN
          DUM=HE(KK,1)
          KLR=KK
        ENDIF
  15  CONTINUE
      NODTRG=NELAB(KLR,NCA)
      RETURN
      END
C=======================================================================
      SUBROUTINE PROF93(JDIAG,NELAB,NELSF,NGLOB,NTTOT,NUMEL,NCVE,NWKCDY)
C
