...
C=========================================================================
C=========================================================================
      SUBROUTINE RACU2D(GNODE,NEL,ID,CORD,SKEF,
...
C=======================================================================
C=========================================================================
      SUBROUTINE RACU3D(GNODE,ALEVO,DESNO,SILE,NEL,
...
C=======================================================================
C=======================================================================
      SUBROUTINE PENTP3(GNODE,NEL,CORD,ID,PRIT,NET,NDIM,IDPRIT,PENALT,
     &IIZLAZ,AMI,ISRPS)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
C      COMMON /PENALL/ PENALT,PRESS
C      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      ...
      DIMENSION GNODE(2,5,*),NEL(NDIM,*),CORD(3,*),ID(5,*)
      DIMENSION PRIT(IDPRIT,*)
      DIMENSION TT21(92),CK(21,3),PJ(3,21),H(21),HP(8),SHEAR(3)
...
C      IF (KLM.LE.8.AND.ID(4,NEL(KLM,NBREL)).NE.0) THEN
      IF (KLM.LE.8) THEN
        TT21(KLM+3*NDIM)=GNODE(2,4,NEL(KLM,NBREL))
C        TT210(KLM+3*NDIM)=GNODE(1,4,NEL(KLM,NBREL))
      ENDIF
        TT21(KLM+3*NDIM+8)=GNODE(2,5,NEL(KLM,NBREL))
...
C
       IUPWIN=0 
       CALL JACT(0.D0,0.D0,0.D0,DET1,CK,0,PJ,HV1,HV2,HV3,H,HP,TT21
     1,DET,SF1,SF2,SF3,NBREL,HXU,HYU,HZU,HXV,HYV,HZV,HXW,HYW,HZW,
     &AMI,NDIM,ISRPS,IIZLAZ,IUPWIN,NEL,SHEAR,ZVXT,ZVYT,ZVZT)

 
C       WRITE(IIZLAZ,*)'ELEMENT= ',NBREL
C       CALL WRRF(TT21,24,'TT21=',IIZLAZ)
C       CALL WRRF(PJ,3*8,'PJ=  ',IIZLAZ)
...
C        WRITE(IIZLAZ,*)'PRIT(1,NBREL)= ',PRIT(1,NBREL),PPP
       ENDDO
...
C END OF LOOP PER ELEMENT    
      END
C=========================================================================
      SUBROUTINE PENTPR(GNODE,NEL,CORD,ID,TT10,PRIT,INDAX,NDIM,
...
C
CE Subroutine PENTP3 is used for postprocessing pressure calculation
CE for 2D analysis when Penalty method is included
C
...
C      COMMON /PENALL/ PENALT,PRESS
      ...
      DIMENSION H(9),ZVHX(9),ZVHY(9),HP(4)
      DIMENSION GNODE(2,5,*),NEL(NDIM,*),ID(5,*),TT10(*),PRIT(IDPRIT,*)
      DIMENSION R(3,3),S(3,3),W(3,3),X(9),Y(9),TT21(36),TT210(36)
      DIMENSION CORD(3,*)
...
C calculation of pressure if we have penalty function
      ...
      DO 450 J=1,IBRGT-1
      K=K+1
      CALL FNTERP(R(IBRGT-1,I),S(IBRGT-1,J),1,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN)
C      WRITE(IIZLAZ,*)'NBREL',NBREL,'PRESS',PRESS
C      PRIT(K,NBREL)=PRIT(K,NBREL)+PRESS
      PRIT(K,NBREL)=PRESS
C        WRITE(IIZLAZ,*)'PRIT(K,NBREL)= ',PRIT(K,NBREL),PRESS
 450  CONTINUE

C       WRITE(IIZLAZ,*)'ELEMENT= ',NBREL
C       CALL WRRF(TT21,12,'TT21=',IIZLAZ)
...
C======================================================================= 
C=========================================================================
      SUBROUTINE ELEM2D(TT1,NEL,ID,NZAD,ZADVRE,CORD,SKEF,TT10,
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

C
CE Subroutine ELEM2D is not used now. It is related to sparce solver.
C
...
C      COMMON /NESTAC/ NSTAC,NKOR,DT
      ...
      CHARACTER*250 NASLOV
      DIMENSION TT1(*),ZADVRE(*),CORD(3,*)
      DIMENSION NEL(NDIM,*),ID(4,*),NZAD(3,*)
      DIMENSION SKEF(NDES,*),TT10(*)
      DIMENSION TABF(2,NTABFT,*),ITFMAX(*)


      DIMENSION X(9),Y(9),LM2(4*9),TT21(4*9),TT210(4*9)
      DIMENSION R(3,3),S(3,3),W(3,3)
      DIMENSION H(9),ZVHX(9),ZVHY(9),HP(4)
      DIMENSION AKVV2(9,9)
      DIMENSION AKMIV2(9,9)
...
C=========================================================================
      ...
      LM2(KLM)=ID(1,NEL(KLM,NBREL))
      LM2(KLM+NDIM)=ID(2,NEL(KLM,NBREL))
      IF (KLM.LE.4) LM2(KLM+2*NDIM)=ID(3,NEL(KLM,NBREL))
      LM2(KLM+2*NDIM+4)=ID(4,NEL(KLM,NBREL))
 130  CONTINUE
C=======================================================================
      DO 140 KLM=1,NDIM
      DO 135 NR=1,4
      IF (ID(NR,NEL(KLM,NBREL)) .NE. 0) THEN
      TT21(KLM+(NR-1)*NDIM)=TT1(ID(NR,NEL(KLM,NBREL)))
      TT210(KLM+(NR-1)*NDIM)=TT10(ID(NR,NEL(KLM,NBREL)))
      ENDIF
 135  CONTINUE
 140  CONTINUE
C
...
C      ENDIF
      ...
      MESTO=NZAD(2,NZDT)
        IF (MESTO.EQ.4) THEN
        TT21(2*NDIM+4+NR)=ZADVRE(NZDT)*FK1
        ELSE
        TT21((MESTO-1)*NDIM+NR)=ZADVRE(NZDT)*FK1
        ENDIF
      ENDIF  
 155  CONTINUE
 160  CONTINUE
...
C reduced integration if penalty function is defined
     ...
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN)
      WDT1=W(IBRGT-1,I)*W(IBRGT-1,J)*DETJ
      DO 190 K=1,NDIM
      DO 185 N=1,NDIM
      AKMIV2(K,N)=AKMIV2(K,N)+PENALT*ZVHX(K)*ZVHX(N)*WDT1
      AKMIV3(K,N)=AKMIV3(K,N)+PENALT*ZVHY(K)*ZVHY(N)*WDT1
      A12(K,N)=A12(K,N)+PENALT*ZVHX(K)*ZVHY(N)*WDT1
      AJV2V3(K,N)=AJV2V3(K,N)+PENALT*ZVHX(K)*ZVHY(N)*WDT1
      A21(K,N)=A21(K,N)+PENALT*ZVHY(K)*ZVHX(N)*WDT1
      AJV3V2(K,N)=AJV3V2(K,N)+PENALT*ZVHY(K)*ZVHX(N)*WDT1
  185 CONTINUE
  190 CONTINUE
  195 CONTINUE
  200 CONTINUE
...
C      CALL ADDSTF(ALEVO,SILE,DESNO,SKEF,F31,MAXA,LM2,NDES,1)
...
C=======================================================================
C=======================================================================
      SUBROUTINE LEFTA(ISPAR,SKEF,ROW,TT1,NEL,ID,NZAD,
     &ZADVRE,CORD,TT10,VVREME,TABF,ITFMAX,NBREL,TIME,SPARC,ISPARC,NKC,
     &KKORAK,MAXAKC)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
	...
	DIMENSION ISPAR(9,*),ROW(*),SPARC(*),ISPARC(2,*),MAXAKC(*)

      DIMENSION TT1(*),NEL(NDIM,*),ID(5,*),CORD(3,*)
      DIMENSION TABF(2,NTABFT,*),TT10(*),ITFMAX(*)
      DIMENSION SKEF(NDES,*),NZAD(3,*),ZADVRE(*),LM2(36)
C
CE Subroutine LEFTA is not used. It is related to sparce solver.
C

      NEQ=JEDN
      NKC=0

C INITIALISATION ROW N-EQUATION

      DO 400 ISPP=1,NEQ
      DO I=1,NEQ
       ROW(I)=0.D0
      ENDDO
      DO 399 ISPP1=1,ISPAR(1,ISPP)
	NBREL=ISPAR(ISPP1+1,ISPP)
      CALL FLEFT(TT1,NEL,ID,NZAD,ZADVRE,CORD,SKEF,TT10,
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME)
...
C      CALL WRR(SKEF,NDES*NDES,'SKEF=')
      
      DO I=1,4*NDIM
        NJEDN=LM2(I)
        IF (NJEDN.NE.0.AND.NJEDN.EQ.ISPP) THEN
          DO J=1,4*NDIM
            NJEDN1=LM2(J)
             IF (NJEDN1.NE.0) THEN
               ROW(NJEDN1)=ROW(NJEDN1)+SKEF(I,J)
             ENDIF
          ENDDO
        ENDIF
       ENDDO
...
C       NODE=NEL(N,NBREL)
C        NJEDN=ID(K,NODE)
C        IF (NJEDN.NE.0.AND.NJEDN.EQ.ISPP) THEN
C          DO J=1,4*NDIM
C       NN=MOD(J,4)
C       KK=J/4+1
C        NODE1=NEL(NN,NBREL)
C         NJEDN1=ID(KK,NODE1)
C             IF (NJEDN1.NE.0) THEN
C               ROW(NJEDN1)=ROW(NJEDN1)+SKEF(I,J)
C             ENDIF
C          ENDDO
C        ENDIF
C       ENDDO
C       
 399  CONTINUE
      DO I=1,NEQ
       IF (I.EQ.ISPP) CALL ZADROW(ISPP,ROW,NZAD,ID)
       IF (DABS(ROW(I)).GT.1.D-12) THEN
         NKC=NKC+1
         SPARC(NKC)=ROW(I)
         ISPARC(1,NKC)=ISPP
         ISPARC(2,NKC)=I
         IF (I.EQ.ISPP) MAXAKC(ISPP)=NKC
       ENDIF
      ENDDO  
 400  CONTINUE
...
C=======================================================================
C=======================================================================
      SUBROUTINE FLEFT(TT1,NEL,ID,NZAD,ZADVRE,CORD,SKEF,TT10,
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME)
      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
      COMMON /VREPER/ NPER,NTABFT
      DIMENSION TT1(*),ZADVRE(*),CORD(3,*),SKEF(NDES,*),TT10(*)
      DIMENSION TABF(2,NTABFT,*)
      DIMENSION NEL(NDIM,*),ID(5,*),NZAD(3,*),ITFMAX(*)

      CALL ELEM2D(TT1,NEL,ID,NZAD,ZADVRE,CORD,SKEF,TT10,
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME)
      
C
CE Subroutine FLEFT is not used. It is related to sparce solver.  
C
...
C=======================================================================
C=======================================================================
      SUBROUTINE SPARS1(ISPAR,NEQ,NEL,ID,NDIM,NPT,NET)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
C      COMMON /DODAT/ NDIMM
C      COMMON /ULAZNI/ IULAZ,IIZLAZ

      DIMENSION ISPAR(9,*)
      DIMENSION NEL(NDIM,*),ID(5,*)

C INITIALISATION ISPAR MATRIX
C
CE Subroutine SPARS is not used. It is related to sparce solver.
C
       ...
       DO NODE=1,NPT
        ...
        IF (NNEQ.NE.0) THEN
         	DO NUMEL=1,NET
           DO J=1,NDIM
            IF(NEL(J,NUMEL).EQ.NODE) THEN
              ISPAR(1,NNEQ)=ISPAR(1,NNEQ)+1
              KK=ISPAR(1,NNEQ)
              ISPAR(KK+1,NNEQ)=NUMEL
            ENDIF
           ENDDO
          ENDDO
         ENDIF
...
C PRINTING MATRIX ISPAR:

C        DO J=1,NEQ
C         WRITE(IIZLAZ,100)(ISPAR(I,J),I=1,NDIM+1)
C        ENDDO 
...
C=======================================================================
C=======================================================================
      SUBROUTINE MSPARC(ISPAR,NKC,JEDN,NDIM)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
	DIMENSION ISPAR(9,*)

C
CE Subroutine MSPARC is not used now. It is related to sparce solver.
C
      NEQ=JEDN
      NKC=0

      DO 400 ISPP=1,NEQ
       NKC=NKC+ISPAR(1,ISPP)*NDIM
 400  CONTINUE
...
C=======================================================================
C=======================================================================
      SUBROUTINE ZADROW(ISPP,ROW,NZAD,ID)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET

      DIMENSION ROW(*),NZAD(3,*),ID(5,*)

C
CE Subroutine ZADROW is not used now. It is related to sparce solver.
C
       
	 DO I=1,NUMZAD
        JJ=ID(NZAD(2,I),NZAD(1,I))
        IF (JJ.EQ.ISPP) ROW(ISPP)=1.D35
       ENDDO
...
C=======================================================================
C=======================================================================
      SUBROUTINE LSPARC(ALEVO,DESNO,MaxKs)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON /ULAZNI/ IULAZ,IIZLAZ
      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET

      DIMENSION ALEVO(*),DESNO(*),MaxKs(*)
      iKc=0

C
CE Subroutine LSPARC is not used now. It is related to sparce solver.
C
...
C=======================================================================
C=========================================================================
      SUBROUTINE FORC2D(GNODE,NEL,ID,CORD,SKEF,
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME,SPSIL,ALEVO,DESNO,SILE,ITER,
     &NGPSIL,MAXA,IBRGT,NASLOV,GUSM,CC,AKT,IIZLAZ,AMI,
     &INDAMI,BETA,TETAO,FB2,FB3,NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,
     &NPER,NTABFT,NDES,IDPRIT,IFORM,PENALT,PRESS,NSTAC,INDAX,IUPWIN,
...
C
CE Subroutine FORC2D is used for calculation forces for 2D analysis
CE It is used global loop per elements
C
...
C STRUJANJA FLUIDA KROZ ELASTICNU CEV
      COMMON /AKIRA/ VOLT,PBALL,VSR,PBOX,PTUBE
C      COMMON /AKIRA/ VOLT,PBALL,VSR,PBOX,PTUBE
      CHARACTER*250 NASLOV
...
C      WDT1=W(IBRGT-1,I)*W(IBRGT-1,J)*DETJ*DEBLJ
      ...
      AJV3V2(K,N)=AJV3V2(K,N)+PENALT*ZVHY(K)*ZVHX(N)*WDT
       ...
       APAXIY(K,N)=APAXIY(K,N)+PENALT*WDT*(ZVHY(K)*H(N)/DEBLJ)
       ENDIF
CC      IF (N.LE.4.AND.PENALT.LT.1.D0) THEN
C       AKV2P(K,N)=AKV2P(K,N)+WDT*(-ZVHX(K)*HP(N))
C       AKV3P(K,N)=AKV3P(K,N)+WDT*(-ZVHY(K)*HP(N))
CC      ENDIF
C      IF (INDAX.EQ.1) THEN
...
C SKEF is local stiffness matrix
      ...
      SKEF(I+NDIM,J+NDIM)=AKMIV3(I,J)+AKVV2(I,J)+AJV3V3(I,J)
      SKEF(I,J+NDIM)=AJV2V3(I,J)
C     SKEF(I+2*NDIM+4,J+2*NDIM+4)=AKK(I,J)+AJK(I,J)
      IF (NSTAC.EQ.0) THEN
C      SKEF(I+2*NDIM+4,J+2*NDIM+4)=SKEF(I+2*NDIM+4,J+2*NDIM+4)+
C     &C(I,J)/TIME
       SKEF(I,J)=SKEF(I,J)+AMV2(I,J)/TIME
       SKEF(I+NDIM,J+NDIM)=SKEF(I+NDIM,J+NDIM)+AMV2(I,J)/TIME
       ENDIF
...
c calculation of interaction forces between fluid and solid
       ...
       DO I=1,NDIM
        DO J=1,NDES
         F31(I)=F31(I)+SKEF(I,J)*TT21(J)
         F31(I+NDIM)=F31(I+NDIM)+SKEF(I+NDIM,J)*TT21(J)
       ENDDO
      ENDDO
       ...
       IF (NSTAC.EQ.0) THEN
        DO I=1,NDIM
         ...
         DO J=1,NDIM
          JJ=J+NDIM
          F31(I)=F31(I)-AMV2(I,J)*TT210(J)/TIME
          F31(II)=F31(II)-AMV2(I,J)*TT210(JJ)/TIME
         ENDDO
        ENDDO
       ENDIF
      ...
      IF (NSTAC.EQ.0) THEN
       DO K=0,2
        DO I=1,NDIM
         ...
         DO J=1,NDIM
          JJ=J+K*NDIM
          F92(II)=F92(II)-AMV2(I,J)*TT210(JJ)/TIME
         ENDDO
        ENDDO
       ENDDO
       ENDIF
