...
C 		PRVRST
C
      SUBROUTINE D3M31(TAU,DEF,IRAC,LPOCG,LPOC1)
      USE PLAST3D
...
C
      ...
      LEMP1=LPOT1 + 1
C
      CALL TAU331(PLAST(LTAU),PLAST(LDEFT),PLAST(LDEFPP),
     1            PLAST(LPOT),PLAST(LEMP),
     1            PLAS1(LTAU1),PLAS1(LDEFT1),PLAS1(LDEFP1),
     1            PLAS1(LPOT1),PLAS1(LEMP1),
...
C =====================================================================
C
      SUBROUTINE TAU331(TAUT,DEFT,SIGST,DALMMT,DALMST,
     1                  TAU1,DEF1,SIGS1,DALMM1,DALMS1,
...
C
      DIMENSION TAUT(*),DEFT(*),TAU(*),DEF(*),TAU1(*),DEF1(*),
     1          SIS1(6),TSE(6,6),DEFL(6),TAUL(6)
      DIMENSION FUN(8,MATE),FUN2(9)
C
      IF(IDEBUG.EQ.1) PRINT *, 'TAU331'
C
CE  INITIAL DATA
C
      TOLF=1.0D-8
...
C
      CALL CLEAR(DEFL,6)
      CALL TRANAL(VMS,TSE,0)
      CALL MNOZI1(DEFL,TSE,DEF,6,6)
C
      DO 11 I = 1,6
      TAUL(I) = 0.D0
      DO 11 J = 1,6 
...
C
      CALL CLEAR(TAU,6)
      CALL MNOZI2(TAU,TSE,TAUL,6,6)
C
CE    TRANSFORM ELAST MATRIX
C
      CALL TRAETP(ELAST,ELAST,TSE)
C
      DO 608 I = 1,6
      TAU1(I) = TAU(I)
  608 DEF1(I) = DEF(I)
