...
C               TI833
C
      SUBROUTINE D8M33(TAU,DEF,IRAC,LPOCG,LPOC1)
...
C
      ...
      LDY1=LDALZ1 + 1*IDVA
      LSIGU1=LDY1 + 1*IDVA
      LSIGP1=LSIGU1 + 1*IDVA
      LTHI1=LSIGP1 + 1*IDVA
C
      CALL TI833(A(LTAU),A(LDEFT),A(LDALXT),A(LDALYT),A(LDALZT),
     &    A(LDYT),A(LSIGCU),A(LSIGCP),A(LTAU1),A(LDEF1),A(LDALX1),
     &    A(LDALY1),A(LDALZ1),A(LDY1),A(LSIGU1),A(LSIGP1),A(LTHI1),
...
C
C=======================================================================
      SUBROUTINE TI833(TAUT,DEFT,DALMXT,DALMYT,DALMZT,DYT,SIGCUT,SIGCPT,
     1          TAU1,DEF1,DALMX1,DALMY1,DALMZ1,DY1,SIGCU1,SIGCP1,THI1,
     1                 FUN,MATE,TAU,DEF,IRAC,THI)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
CS     PODPROGRAM ZA INTEGRACIJU KONSTITUTIVNIH RELACIJA ZA 
CS     MATERIJALNI MODEL MISICA
C
...
C
        ...
        TAU(2)=TAUT(2)+DLSIG2
C
CS !!!! RELAKSACIJA - PUZANJE !!!!
C
      IF(KOR.GE.INDREL.AND.ITER.GE.1) THEN
C
        VREMET=VREME-DT
        DCREEP=0.5D0*(DALMXT+DLAMDX)
C
CS PROVERA DA LI JE TEKUCI STRETCH U ZADATOM OPSEGU KRIVIH - ZA PUZANJE
C
      	IF (DCREEP.LT.DLAMDC(1,1).OR.DCREEP.GT.DLAMDC(NLAM,1)) THEN
          WRITE(3,2230) DCREEP
C 2230     FORMAT(/' ','KRIVA PUZANJA NAPON-VREME NIJE DEFINISANA ZA OVU 
C     & VREDNOST STRECA'/,' STREC =',E12.5)
 2230     FORMAT(/' ','RELAXATION CURVE STRESS-TIME IS NOT DEFINED FOR  
     & THIS VALUE OF STRETCH'/,' STRETCH =',E12.5)
          STOP
        ENDIF
C
CS PETLJA ZA LAMDA
C
       DO 321 J = 1,NLAM
C
        IF (DCREEP.GE.DLAMDC(J,1).AND.DCREEP.LE.DLAMDC(J+1,1)) THEN
          DLAM1=DLAMDC(J,1)
          DLAM2=DLAMDC(J+1,1)
C
CS PETLJA ZA KRIVU NAPON-VREME 
C
CS SIGMA NA KRIVI 1, U TRENUTKU T
C
          SIGM1T=DLAMDC(J,2)+DLAMDC(J,3)*DEXP(DLAMDC(J,4)*VREMET)
C
C
CS SIGMA NA KRIVI 1, U TRENUTKU T+DT
C
          SIG1DT=DLAMDC(J,2)+DLAMDC(J,3)*DEXP(DLAMDC(J,4)*VREME)
C         
CS SIGMA NA KRIVI 2, U TRENUTKU T
C
          SIGM2T=DLAMDC(J+1,2)+DLAMDC(J+1,3)*DEXP(DLAMDC(J+1,4)*VREMET)
C
CS SIGMA NA KRIVI 2, U TRENUTKU T+DT
C
          SIG2DT=DLAMDC(J+1,2)+DLAMDC(J+1,3)*DEXP(DLAMDC(J+1,4)*VREME)
C
CS UZDUZNE K-KE
C
       ETC1=DABS(DLAMDC(J,3)*DLAMDC(J,4)*DEXP(DLAMDC(J,4)*VREME))
       ETC2=DABS(DLAMDC(J+1,3)*DLAMDC(J+1,4)*DEXP(DLAMDC(J+1,4)*VREME))
C
CS POPRECNE K-KE
C
       ETCP1=DABS(DLAMDC(J,3)*DLAMDC(J,4)*DEXP(DLAMDC(J,4)*VREME))
       ETCP2=DABS(DLAMDC(J+1,3)*DLAMDC(J+1,4)*DEXP(DLAMDC(J+1,4)*VREME))
C
      	  ET(1) = (ETC2-ETC1) * (DCREEP-DLAM1) / (DLAM2-DLAM1) + ETC1
      	  ET(2) = (ETCP2-ETCP1) * (DCREEP-DLAM1) / (DLAM2-DLAM1) + ETCP1
C
          SIGCP=ET(1)*DT
          SIGCU=ET(2)*DT
C
...
C
          TAU(1)=TAU(1)-SIGCP
          IF(TAU(2).GT.SIGCU) TAU(2)=TAU(2)-SIGCU
C
C
CS KOREKCIJA TANGENTNOG MODULA ZBOG PUZANJA 
C
          SIGMPR(1,1)=SIGMPR(1,1)+0.5D0*DT*(ETC2-ETC1)/(DLAM2-DLAM1)
C          SIGMPR(2,2)=SIGMPR(2,2)+0.5D0*DT*(ETCP2-ETCP1)/(DLAM2-DLAM1)
C
          GO TO 349
C
         ENDIF
